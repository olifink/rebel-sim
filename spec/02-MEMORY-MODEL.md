...
- **`BANK@` and `BANK-SIZE` MUST both be `IMMEDIATE`, dual-mode on
  `STATE`** (found M53, Oliver, trying `: TESTING BANK@ CHAR ;`;
  `BANK-SIZE` hit the identical bug later, trying `: FOO BANK-SIZE
  SYSV ;`, and got the identical fix): resolve the name at the time the
  word itself runs, always — while compiling, that's *now*, at the
  containing definition's own compile time, and the resolved value
  (base address for `BANK@`, size for `BANK-SIZE`) gets compiled in as
  a `LIT` (`04-FORTH-CORE.md` §5.3's "baked into the definition"
  pattern, same category as `S"`/`."`/`(`, though those bake in raw
  text rather than a resolved value); while interpreting, the value is
  simply pushed, unchanged from before. A plain non-`IMMEDIATE` version
  of either word compiles a call to itself and leaves the *outer
  compiler's own* token loop to process whatever token follows — which
  was never meant to be looked up as an ordinary word at all, so
  compilation fails right there, before the containing definition can
  ever run. This makes the resolved value a compile-time constant of
  the *defining* line, not a fresh per-call lookup — correct for the
  fixed system banks and any bank already stable when the defining word
  is compiled, stale if that bank is later dropped and recreated at a
  new address or size.
...
Fixed 36 bytes at the start of the `MMAP` bank (i.e. at the owning
arena's offset `0`):
...
| 28 | 1 cell | `INK` | Boot-time `SCREEN.INK` sysvar value — same palette-index-or-literal-RGB convention that sysvar already uses (§4.6). Lets a distinctly-`Personality`'d arena boot into visibly different colors, a cheap and immediate signal of "this is a different machine." |
| 32 | 1 cell | `PAPER` | Boot-time `SCREEN.PAPER` sysvar value, same convention as `INK` above. |
...
`MMAP`'s raw content requirement is `36 + MAX_SLOTS × 24` bytes (36 —
not 16 — since §5.1's header now includes all of `Personality`,
`INK`/`PAPER` included) — a fixed function of a build-time constant.
**[Revised]** an earlier version of this spec exempted `MMAP` from
§4.3's size-class rule entirely, treating this computed byte count as
the bank's actual, exact size. That exemption is gone: `MMAP`'s
declared size **MUST** be this raw requirement rounded up to the
smallest §4.3 size class that fits, exactly like any other carved
bank's content-driven request (§4.3's own "this rule applies uniformly
to every source of a bank-size request" already covered this case;
`MMAP` was simply never routed through it). With the recommended
default of `MAX_SLOTS = 64`, the raw requirement is 1572 bytes,
comfortably inside the smallest class, **`MIN_BANK_SIZE`** — so
`MMAP`'s declared size is `MIN_BANK_SIZE` bytes, not 1572 (1564 bytes
with the pre-`INK`/`PAPER` 28-byte header; 1552 bytes with the original
pre-`Personality` 16-byte header; 4096 bytes prior to M58's §4.3 floor
change; 2048 bytes as of M58). A target with a larger `MAX_SLOTS`
recomputes the same way; only a `MAX_SLOTS` whose raw requirement
exceeds the current floor (a little above 83 slots, as of the 2 KiB
floor and 36-byte header) would round to a larger class instead.
...
`MAX_SLOTS = 64` (`MMAP`'s raw requirement, `36 + 64×24 = 1572` bytes,
