...
`BANK@`, `BANK-SIZE`, `CREATE-BANK`, `PROJECT`, `RESTORE`, `BSAVE`,
`BLOAD`, §6) need
...
definition that contains it** (`S"`, `."`, `(`, `BANK@`, `BANK-SIZE`,
and the `…-XT`-resolving use of `'` in §6.5's control-flow layer) needs
the opposite: it must run at *that containing definition's own compile
...
`BANK@`/`BANK-SIZE` (`02-MEMORY-MODEL.md` §4.7) are the odd ones out in
this second group: each bakes in a resolved *value* (the bank's base
address or size, compiled as `LIT`), not raw text like `S"`/`."` do —
found M53 for `BANK@`, and again later for `BANK-SIZE` on the same
bug, after a plain non-`IMMEDIATE` version of either word inside a
colon definition left its own compiler trying (and failing) to look up
the following name token as an ordinary word.
...
| `(` | `( -- )`, comment to matching `)` | **KERNEL, IMMEDIATE** | Same raw-consumption mechanism as `S"`, and the same reason it must carry `IMMEDIATE`: a `(` comment written inside a colon-definition's own source has to be consumed at that definition's own compile time, not deferred to whenever the definition is later called. Discarded, compiling or not — classic Forth behavior. **[Revised]** an earlier version of this row specified retaining the comment as compiled inline data so a decompiler (`SEE`) could still show it; Rebel-Sim shipped and then reverted that (M11, M44) once `SEE` proved unable to render it back as `( ... )` syntax at all — it printed indistinguishably from a genuine string a program discards on purpose, which defeated the entire point of retaining it. Does not nest — the first closing `)` always ends it, a documented limitation, not a bug: any token *ending in* a close-paren character, not just a standalone one, closes it (`consumeQuotedText`'s actual matching rule, checked against Rebel-Sim's implementation) — a nested aside glued onto a word (e.g. writing a token id as `148)` with no separating space) closes the comment early and leaks the remainder of the line into the dictionary as live code. This has independently bitten Rebel-Sim's own `system.fth` three times (M46, M48, M68) purely from long descriptive comments, never from a program's own use of `(`. |
| `\` | `( -- )`, comment to end of the current input line | **BOOTSTRAP** | `: \ BEGIN BL WORD NIP 0= UNTIL ; IMMEDIATE` — loops calling `WORD` (§6.13, already native) with `BL` as the delimiter until it returns a zero length, which is `WORD`'s own already-specified "line exhausted" signal; needs no new primitive. **Added specifically to give `(`'s nesting limitation above a safe alternative**: a rest-of-line comment has no closing token of any kind to glue an aside onto, so it cannot suffer `(`'s failure mode — reordering this table's own history, this row is what §9's "not specified here either; added only if a real need surfaces" note used to say about `\`, before repeatedly hitting that need firsthand settled it. `IMMEDIATE` for the same reason `(` carries it — written inside a colon-definition, its text must be discarded at that definition's own compile time. |
...
| `DUMP` | `( addr -- )` — classic hex dump: 16 rows of 8 bytes, each an 8-digit hex address, 8 space-separated 2-digit hex bytes, then those same 8 bytes again as characters, with anything below `BL` shown as `.`. Fixed 128-byte extent, no length argument. Built on three small helpers of its own — `HEXDIGIT`/`HEX2`/`HEX8` — which derive hex formatting entirely from `/`/`MOD`, needing no shift primitive. No bounds checking against the arena's real extent, the same trust-the-caller convention `@`/`C@`/`BANK@` already carry. | `@`, `C@`, `EMIT`, `/`, `MOD` |
...
- **`LOAD` / classic screen-source interpretation** — reading a
  ...
  scratch.
- ~~**Comment nesting/`\`.**~~ **Resolved.** `(` (§6.7) still does not
  nest — the first token ending in a close-paren character always ends
  it, a documented limitation, not a bug. `\` (rest-of-line comment) is
  no longer unspecified: §6.7 now gives it a real BOOTSTRAP definition,
  added once `(`'s nesting limitation surfaced as a real, repeated need
  (Rebel-Sim's own `system.fth` hit it three separate times) rather than
  a hypothetical one.
